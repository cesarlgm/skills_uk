...
\section{Estimation}
We estimate the three-equation model:
\beqn
...
\Delta \overline{ \ln S_{ijet}}&=&\sum_k \theta_{ke}\overline{S_{kejt}} \pi_{kjt}-\pi_{ijt} \\
1&=&\sum_k\theta_{ke}\overline{S_{kejt}}  \label{eq:total_sum} \\
\Delta  \left[\ln \frac{q_{ejt}}{q_{e'jt}}\right]&=&\beta_j\left[\sum_k\left(\theta_{ke} \overline{S_{kjet}}-\theta_{ke'} \overline{S_{kje' t}}\right)\pi_{kjt}\right]+\gamma_{e,e't} \label{eq:employment_eq} 
\eeqn
where $\pi_{kjt}=\frac{\sigma_j}{\sigma_j-1}\Delta \ln A_{kjt}$ and $\beta_j=\frac{\sigma_j-1}{\sigma_j}$. 

Let $\varepsilon_l$ be vector of stacked errors from equation $j$ and let $\varepsilon$ the vector of stacked errors of the full model ($\varepsilon=(\varepsilon_1,\varepsilon_2,\varepsilon_3)'$). We estimate the model parameters $\mu$ by GMM, minimizing the quadratic form:
\beqn
	Q=\varepsilon(\mu)'Z'Z\varepsilon(\mu)
\eeqn 
where $Z$ is the vector of instruments.
\subsection{Instruments}
The matrix of instruments $Z$ is a block-diagonal matrix with diagonal elements given by the matrix of instruments for each equation ($Z_l$). 
\bitem
	\item \textbf{Equation 1 instruments:} we instrument the skill levels $\overline{S_{kejt}}$ with the size-weighted skill level of the other education groups in that job:
	\beqns
		\overline{S_{k-ejt}}=w_{e'jt}\overline{S_{ke'jt}}+w_{e''jt}\overline{S_{ke''jt}}
	\eeqns
	where $w_{e'jt}$ is the employment share of education $e'$ in that job and year. We also include as instruments the appropriate set of --negative-- job by skill by year dummies. Therefore, $Z_2$ is a $12JT\times(12+4JT)$ matrix.
	\item \textbf{Equation 2:} instruments for this equation are the skill levels by education level. Therefore, $Z_2$ is a $3JT\times12$ matrix.
	\item \textbf{Equation 3 instruments:} there are three types of instruments for this equation: education pair dummies, job by year dummies, and job by education dummies. Thus, $Z_3$ is a $2JT\times(6+JT+JE)$ matrix. 
\eitem
\subsection{Identification}
The GMM model is overidentified. In total we have $30 + 5JT + JE$ instruments to identify $18+4JT+J$ parameters.

\subsection{Standard errors}
The GMM estimates are distributed as follows:
\beqns
	\sqrt{N}(\hat\mu-\mu)\rightarrow N(0,V)
\eeqns
where $(D'D)^{-1}D'VD(D'D)^{-1}$, with $D$ the model gradient, and
\beqns
	\hat{V}=\frac{1}{N}\sum_{n=1}^N\varepsilon_n(\hat\mu)\varepsilon_n(\hat\mu)'
\eeqns
	\subsection{Calculating the gradient}
	\paragraph{Reminder:}
	\beqns
		\frac{\partial Q}{\partial x}=x'(A+A')
	\eeqns
	I can write the quadratic from as:
	\beqns
	 Q(\gamma)&=&\frac{1}{N}\left(u(\gamma)'Z(Z'Z)^{-1}Z'u(\gamma)\right)
	\eeqns
	then,
	\beqns
		\frac{\partial Q(\gamma)}{\partial \gamma}&=&\frac{2}{N}\left(\frac{\partial u(\gamma)}{\partial \gamma}\right)'Z(Z'Z)^{-1}Zu(\gamma)
	\eeqns
	now, I start element by element:
	\beqns
		\frac{\partial \Delta \ln S_{iejt}}{\partial \ln A_{kjt}}&=&\theta_{ke}S_{kejt}\\
		\frac{\partial \Delta \ln S_{iejt}}{\partial \ln A_{ijt}}&=&\theta_{ie}S_{iejt}-1\\
		\frac{\partial \Delta \ln S_{iejt}}{\partial \ln \theta_{ke}}&=&S_{kjt}\Delta \ln A_{kjt}
	\eeqns
	for the sum to 1 restrictions we have:
	\beqns
		\frac{\partial g_{ejt}(\gamma)}{\partial \theta_{ke}}&=&-S_{kejt}
	\eeqns
	\red{next thing to do: write the gradient}
...
%\subsection{Averages}
%\input{../results/figures/average_manual.tex}
%\input{../results/figures/average_routine.tex}
%\input{../results/figures/average_social.tex}
%\input{../results/figures/average_abstract.tex}
%\FloatBarrier
%\subsection{Distributions}
%\input{../results/figures/distribution_manual.tex}
%\input{../results/figures/distribution_routine.tex}
%\input{../results/figures/distribution_social.tex}
%\input{../results/figures/distribution_abstract.tex}
%
