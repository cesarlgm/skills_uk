...
\beqns
\Delta \overline{ \ln S^e_{ijet}}&=&\frac{\sigma_j}{\sigma_j-1} \left(\sum_k \theta_{ke}\overline{S_{kejt}} \Delta \ln A_{kjt}-\Delta \ln A_{ijt}\right)+\varepsilon_{iej} \label{eq:skills_eq} 
\eeqns
...
\Delta  \left[\ln \frac{q_{eJt}}{q_{e'Jt}}\right]&=&\beta_j\left[\sum_k\left(\theta_k^e \overline{S_{kJet}}-\theta_k^{e'} \overline{S_{kJe' t}}\right)\pi_{kjt}\right]+const_{e,e't} \label{eq:employment_eq} 
\eeqns
...
\bitem
	...
	\beqn
		\min_{\theta} \frac{1}{N}\left(u'Z(Z'Z)^{-1}Z'u\right)
	\eeqn
