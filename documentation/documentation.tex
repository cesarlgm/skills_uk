...
\beqns
	...
	\Lambda&=&\left(\Delta \ln A_{1t}^l, \Delta \ln A_{2t}^l, \Delta \ln A_{3t}^l,\Delta \ln A_{1t+1}^l \dots \Delta \ln A_{3T}^h\right)
\eeqns
...
\label{eq:total_sum}
1&=&\sum_k\theta_k^e\overline{S_{keJt}}+\nu_{jet}\\
\label{eq:employment_eq}
\Delta  \left[\ln \frac{q_{eJt}}{q_{e'Jt}}\right]&=&\sum_k\left(\theta_k^e \overline{S_{kJet}}-\theta_k^{e'} \overline{S_{kJe' t}}\right)\Delta \ln A_{kjt}+const_{e,e't}+\eta_{ee'jt}
\eeqn
\subsection{Moment equations}
We need to estimate:
\bitem
	\item $E\times K$ investment cost parameters $\theta$
	\item $(K-1)\times J\times T$ productivity change terms $\Delta \ln A_{kjt}$
	\item $J$ elasticity of substitution parameters $\sigma_j$.
	\item $3\times T$ education pair constants.
\eitem 
Let us count moments. Let $\iota_K$ be a vector of ones of dimension $K$. Define $Z'_{jet}=\iota_K\otimes[\overline{S_{1eJt}}\dots \overline{S_{KeJt}}]$. From \eqref{eq:skills_eq} we have $K\times J\times T$ moment equations:
\beqn
	\expect{Z_{jet}\varepsilon_{jet}}{} 
\eeqn
from \eqref{eq:total_sum} we have $K\times E\times J\times T$ moments:
\beqn
\expect{Z_{jet}\nu_{jet}}{} 
\eeqn
Finally, from equation \eqref{eq:employment_eq} we obtain $J\times T$ equations. \red{is this right?}.
