function [e1_d_ln_a_index,e1_occ_index,e1_educ_index, ...
    e3_a_index, e3n_educ_index,e3d_educ_index,e1_code,e3_code]=...
    get_occ_indexes(data)
    names=transpose(data.Properties.VariableNames);

    eqn_1_indexes=names(startsWith(names,"a_"));
    eqn_2_indexes=names(startsWith(names,"ts_"));
    ...
    e1_year_index=str2double(splitted_name(:,6));
    e1_skill_index=str2double(splitted_name(:,4));
    e1_occ_index=str2double(splitted_name(:,5));
    e1_educ_index=(str2double(splitted_name(:,3))-1)*4+e1_skill_index;


    e1_code=e1_occ_index*100000+e1_year_index*10+e1_skill_index;

    %dlnA_index gives the indexes in the dlnA vector
    [dlnA_index,~,e1_d_ln_a_index]=unique(e1_code);
    ...
    [~,e3_a_index]=ismember(e3_code,dlnA_index);
