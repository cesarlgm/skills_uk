...
graphAlpha <- .2
#Original indexes
heatmap <- ggtern(graphData,mapping=aes(x=analyticalIndexOr, y=RIndexOr, z=manualIndexOr,color=period)) +
  stat_density_tern(aes(fill=period),alpha=graphAlpha,bins=3, geom='polygon',weight=graphData$occObs)+
  theme_bw() +
...
pdfName <- paste("output/densitySep","TimeAgeOldOr.pdf",sep="")
ggsave(pdfName,heatmap,dpi = 700,width=10,height=height)


#Dummy routine PC use
heatmap <- ggtern(graphData,mapping=aes(x=analyticalIndexDum, y=routinePCIndexDum, z=newManualIndexDum,color=period)) +
  stat_density_tern(aes(fill=period),alpha=graphAlpha,bins=3, geom='polygon',weight=graphData$occObs)+
  theme_bw() +
  scale_fill_manual(values=c( "#0859AF", "#F5B041", "#922B21"))+
  scale_color_manual(values=c( "#17379B" ,"#9B6817", "#651F0C"))+
...
pdfName <- paste("output/densitySep","TimeAgeOldDum.pdf",sep="")
ggsave(pdfName,heatmap,dpi = 700,width=10,height=height)
...
pdfName <- paste("output/densitySep","TimeAgeOldCont.pdf",sep="")
ggsave(pdfName,heatmap,dpi = 700,width=10,height=height)
...
pdfName <- paste("output/densitySep","TimeAgeOldMod.pdf",sep="")
ggsave(pdfName,heatmap,dpi = 700,width=10,height=height)
